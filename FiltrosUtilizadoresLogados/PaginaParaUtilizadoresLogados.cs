...
namespace CliSenhas2024.Filtros_UtilizadoresLogados
{
    public class PaginaParaUtilizadoresLogados : ActionFilterAttribute  //F12. Essa biblioteca tem o metodo OnActionExecuting, e toda vez que cai nesse metodo,
                                                        //antes de executar qualquer ação dos controladores, primeiro cai nesse metodo
    {
                                                                               ...
                                                                               // void pois n retorna nada
        {
            ...
            {
                context.Result = new RedirectToRouteResult(new RouteValueDictionary { { "controller", "RestritoUtilizadores" }, { "action", "IndexRestritoUtilizador" } });
            }
            else
            {
